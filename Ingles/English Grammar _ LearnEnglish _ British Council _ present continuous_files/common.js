...
function setExerciseMode() {
    ...
    $('#nextExerciseButton').addClass('disabled hidden'); //('hidden');
    ...
    $('body').removeClass('mode-normal mode-default mode-placement_test mode-learn_mode mode-test_mode');
    $('body').addClass('mode-' + gameMode).addClass('type-' + gametype); //.addClass('skin-' + gameskin);
    ...
    */
    ...
    if (getMode() != "placement_test" && gameOverview && requireTabs) {
        ...
        if (gameOverview.image && gameOverview.image.length > 0) {
            ...
            var element = '<img src="' + actualImageNode.text() + '" class="center-block img-responsive" alt="' + alt_tag + '" title="' + alt_tag + '">' + expand_button; //width="320" height="240"
...
function onFinish(obj) {
    ...
    if (remaining > 0) {
        ...
        if (getMode() == "placement_test") {
            ...
            {
                str = "You have not anwsered the question. Do you want to proceed?";
            } else {
...
function onConfirmedFinish() {
    ...
    } else if (getMode() == "placement_test") {
        showFeedback = false;
        showTryAgain = false;
    }
...
function getRemainingQuestionsCount() {
    ...
    if (getMode() == "learn_mode") {
		...
		*/
        ...
        if (gametype == "GapFillDragAndDrop" || gametype == "Grouping" ||
            gametype == "Matching" || gametype == "ImageMatching" ||
            gametype == "ReorderingHorizontal" || gametype == "ReorderingVertical") {
            // drag n drop type
            remaining = $('.option-pool-list').children().length; //size();

        } else if (gametype == "GapFillTyping") {
        ...
        } else if (gametype == "GapFillDropDown") {
            // typing / dropdown type
            $('[name=gapfield]').each(function(index) {
                if ($.trim($(this).text()) == "") {
                    remaining++;
        ...
        } else if (gametype == "TrueOrFalse" || gametype == "MultipleChoice" || gametype == "MultipleSelection") {
        ...
        } else if (gametype == "HighlightSelection" || gametype == "HighlightDeletion") {
...
function createSimpleAudioButton(url) {
    var str = '';
    ...
    if (url) {
        str = '<a href="javascript:;" class="btn-sm btn-primary mini-audio" data-href="' + url + '"><span class="glyphicon glyphicon-play"></span><span class="sr-only">Toggle Audio</span></a>';
